...
public class MLSignUpActivity extends MLBaseActivity {
    ...
    private void restSignUp() {
        ...
        mDialog.setMessage(getString(R.string.ml_sign_up_begin));
        mDialog.show();
        new Thread(new Runnable() {
            @Override public void run() {
                ...
                try {
                    JSONObject object =
                            MLNetworkManager.getInstance().createUser(mUsername, mPassword);
                    int errorCode = object.optInt("code");
                    String errorMsg = object.optString("msg");
                    if (errorCode != 0) {
                        Snackbar.make(getRootView(), getString(R.string.ml_sign_up_failed)
                                + " - "
                                + errorCode
                                + " - "
                        ...
                        @Override public void run() {
                            ...
                            switch (errorCode) {
                                ...
                                case EMError.USER_ALREADY_EXIST:
                                    error = getString(R.string.ml_error_user_already_exits);
                                    break;
                                // 参数不合法，一般情况是username 使用了uuid导致，不能使用uuid注册
                                case EMError.USER_ILLEGAL_ARGUMENT:
                                    error = getString(R.string.ml_error_user_illegal_argument);
                                    break;
                                // 服务器未知错误
                                ...
                                case EMError.USER_REG_FAILED:
                                    error = getString(R.string.ml_error_user_reg_failed);
                                    break;
                                default:
                                    error = getString(R.string.ml_sign_up_failed);
                                    break;
                            }
