...
public class ConnectionListener implements EMConnectionListener {
    ...
    @Override public void onConnected() {
        VMLog.d("onConnected");
        Hyphenate.getInstance().unBuildToken = true;
        // 设置链接监听变化状态
        ConnectionEvent event = new ConnectionEvent();
        event.setType(Constants.CONNECTION_CONNECTED);
    ...
    @Override public void onDisconnected(final int errorCode) {
        ...
        Hyphenate.getInstance().unBuildToken = false;
        ConnectionEvent event = new ConnectionEvent();
        if (errorCode == EMError.USER_LOGIN_ANOTHER_DEVICE) {
            VMLog.d("user login another device - " + errorCode);
            Hyphenate.getInstance().signOut(null);
            // 设置链接监听变化状态
            event.setType(Constants.CONNECTION_USER_LOGIN_OTHER_DIVERS);
        } else if (errorCode == EMError.USER_REMOVED) {
            VMLog.d("user be removed - " + errorCode);
            Hyphenate.getInstance().signOut(null);
            // 设置链接监听变化状态
            event.setType(Constants.CONNECTION_USER_REMOVED);
        } else {
            VMLog.d("con't servers - " + errorCode);
            // 设置链接监听变化状态
            event.setType(Constants.CONNECTION_DISCONNECTED);
        }
        // 发送订阅消息，通知网络监听有变化
        EventBus.getDefault().post(event);
    }
